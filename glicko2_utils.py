...
# The `FullRating` type represents a rating, deviation, and volatility.
FullRating = Tuple[float, float, float]
...
def _get_v(mu: float, results: ResultList) -> float:
    ...
    The ratings and rating deviations in `results` should all be in the
    Glicko-2 rating system's scale.
    """
...
def _get_sigma_p(mu: float, phi: float, sigma: float,
                 results: ResultList) -> float:
...
def update_deviation(deviation: float,
                     volatility: float,
                     time: float = 1) -> float:
...
def win_pct(rating_1: float, deviation_1: float, rating_2: float,
            deviation_2: float) -> float:
